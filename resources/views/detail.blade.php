...
<head>
    ...
    <title>Detail {{ $wisata->nama_wisata }} - XPloreJogja</title>
    ...
    <main class="container mx-auto px-6 py-10 max-w-5xl">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mb-16">
            <div class="border-l-4 border-cyan-100 pl-6">
                <h3 class="font-bold border-b-2 border-gray-800 w-fit mb-5 pb-1">Operasional</h3>
                <div class="inline-flex w-fit bg-cyan-600 text-white rounded-xl overflow-hidden text-center text-sm shadow-md">
                    <div class="py-3 px-5 border-r border-cyan-500 italic">Buka<br>{{ $wisata->jam_buka }}</div>
                    <div class="py-3 px-5 italic">Tutup<br>{{ $wisata->jam_tutup }}</div>
                </div>
                @if($current_parent == 'Kuliner' && $wisata->info_tiket_tambahan)
                <div class="text-[11px] text-gray-500 leading-tight mt-3">
                    <p class="font-semibold mb-1 italic">Catatan Jam Operasional:</p>
                    <p>{!! nl2br(e($wisata->info_tiket_tambahan)) !!}</p>
                </div>
                @endif
            </div>

            <div class="border-l-4 border-cyan-100 pl-6">
                <h3 class="font-bold border-b-2 border-gray-800 w-fit mb-5 pb-1">Harga Tiket</h3>
                <div class="inline-block w-fit bg-cyan-600 text-white py-3 px-6 rounded-xl text-sm font-bold mb-3 shadow-md">
                    {{ $wisata->harga_tiket }}
                </div>
                @if($current_parent != 'Kuliner' && $wisata->info_tiket_tambahan)
                <div class="text-[11px] text-gray-500 leading-tight">
                    <p class="font-semibold mb-1 italic">Catatan:</p>
                    <p>{!! nl2br(e($wisata->info_tiket_tambahan)) !!}</p>
                </div>
                @endif
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-10 text-sm border-t border-gray-100 pt-8">
                ...
                <div class="space-y-3">
                    <p class="font-bold flex items-center gap-2 text-gray-800">
                        <i class="fas fa-check-circle text-cyan-600 text-lg"></i> Area Parkir
                    </p>
                    <ul class="text-xs text-gray-500 ml-7 space-y-1.5 italic">
                        @if($wisata->biaya_parkir)
                            @foreach(explode("\n", $wisata->biaya_parkir) as $parkir)
                                @if(trim($parkir))
                                    <li><i class='fas fa-chevron-right text-[9px] mr-1'></i> {{ trim($parkir) }}</li>
                                @endif
                            @endforeach
                        @else
                            <li><i class='fas fa-chevron-right text-[9px] mr-1'></i> Tersedia</li>
                        @endif
                    </ul>
                </div>

                <!-- Fasilitas Lainnya (Dinamis dari Input 'Fasilitas' dipisah koma, support format Nama=Rincian) -->
                @if($wisata->fasilitas)
                    @foreach(explode(",", $wisata->fasilitas) as $fasil)
                        @php
                            $fasil = trim($fasil);
                            $parts = explode('=', $fasil, 2);
                            $namaFasil = trim($parts[0]);
                            $rincianFasil = isset($parts[1]) ? trim($parts[1]) : null;
                        @endphp
                        @if($namaFasil)
                        <div class="space-y-3">
                            <p class="font-bold flex items-center gap-2 text-gray-800">
                                <i class="fas fa-check-circle text-cyan-600 text-lg"></i> {{ $namaFasil }}
                            </p>
                            @if($rincianFasil)
                            <ul class="text-xs text-gray-500 ml-7 space-y-1.5 italic">
                                <li><i class='fas fa-chevron-right text-[9px] mr-1'></i> {{ $rincianFasil }}</li>
                            </ul>
                            @endif
                        </div>
                        @endif
