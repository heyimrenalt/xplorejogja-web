...
<head>
    ...
    <title>{{ $category->name ?? 'Blog & Informasi' }} - XPloreJogja</title>
...
@include('partials.navbar-static', ['activeCategory' => 'blog-informasi'])
...
<main class="container mx-auto px-6 py-10 min-h-screen">
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-cyan-800">{{ $category->name ?? 'Blog & Informasi' }}</h1>
    ...
    @if($subKategoris->count())
        ...
        <section class="mb-12">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <a href="{{ $wisata->link_sumber ?? '#' }}" target="_blank" rel="noopener noreferrer"
                   class="block bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:-translate-y-1 transition group">
                    ...
                    <div class="p-4 flex justify-between items-center bg-gray-50/50">
                        <div>
                            ...
                            <p class="text-xs text-gray-500">{{ $wisata->alamat_lengkap ?? '-' }}</p>
                        </div>
                        <i class="fas fa-external-link-alt text-orange-400"></i>
                    </div>
                </a>
                @empty
               ...
               class="block bg-white rounded-2xl overflow-hidden shadow-[0_8px_20px_-6px_rgba(0,0,0,0.2)] border border-gray-100 transition hover:-translate-y-1 group">
                ...
                <div class="p-4 flex justify-between items-center bg-gray-50/50">
                    <div class="min-w-0 mr-2">
                        <h4 class="text-sm font-bold group-hover:text-cyan-600 transition-colors line-clamp-2">{{ $wisata->nama_wisata }}</h4>
                        <p class="text-xs text-gray-500">{{ $wisata->alamat_lengkap ?? '-' }}</p>
                    </div>
                    <i class="fas fa-external-link-alt text-orange-400 shrink-0"></i>
                </div>
            </a>
            @endforeach
        </div>
